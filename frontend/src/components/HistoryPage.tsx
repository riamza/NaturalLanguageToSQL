  ...
  return (
    <div className="page-container fade-in">
      <div className="page-header">
        ...
        <p className="page-description">Review past natural language queries, their resulting SQL, and performance metrics.</p>
      </div>
          ...
          history.map((item) => (
            <div key={item.id} className="history-card" onClick={() => setSelectedItem(item)}>
              <div className="history-card-header">
                ...
                <div className={`status-badge ${item.isSuccessful ? "success" : "error"}`}>
                  {item.isSuccessful ? "Success" : "Failed"}
                </div>
              </div>
              <div className="history-card-footer">
                <span className="time">{new Date(item.createdAtUtc).toLocaleString()}</span>
                <span className="duration">{(item.executionTimeMs / 1000).toFixed(2)}s</span>
              </div>
            </div>
          ))
        )}
        ...
        <div className="modal-overlay" onClick={() => setSelectedItem(null)}>
          <div className="modal-content premium-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="close-modal" onClick={() => setSelectedItem(null)} aria-label="Close">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="modal-body">
              <div className="modal-section">
                <span className="modal-section-title">Natural Language Prompt</span>
                <div className="modal-text-block highlight-block">{selectedItem.prompt}</div>
              </div>
              ...
              {!selectedItem.isSuccessful && selectedItem.errorMessage && (
                <div className="modal-section">
